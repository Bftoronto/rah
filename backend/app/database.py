...
def create_indexes():
    ...
    try:
        with engine.connect() as conn:
            # Индексы для поездок
            conn.execute("CREATE INDEX IF NOT EXISTS idx_rides_driver_id ON rides(driver_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_rides_passenger_id ON rides(passenger_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_rides_status ON rides(status)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_rides_date ON rides(date)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_rides_from_location ON rides(from_location)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_rides_to_location ON rides(to_location)")
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_users_is_driver ON users(is_driver)")
            
            # Индексы для чатов
            conn.execute("CREATE INDEX IF NOT EXISTS idx_chats_ride_id ON chats(ride_id)")
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_chat_messages_user_from_id ON chat_messages(user_from_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_chat_messages_user_to_id ON chat_messages(user_to_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_chat_messages_timestamp ON chat_messages(timestamp)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_chat_messages_is_read ON chat_messages(is_read)")
            
            # Индексы для платежей
            conn.execute("CREATE INDEX IF NOT EXISTS idx_payments_user_id ON payments(user_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_payments_ride_id ON payments(ride_id)")
